...
class TestRewardConfiguration:
    ...
    def test_reward_tuning_values(self):
        ...
        assert cfg["w_track_ang_vel"] == pytest.approx(2.0)
        assert cfg["w_flat_orientation"] == pytest.approx(-0.9)
        assert cfg["w_pose"] == pytest.approx(0.5)
        ...
        assert cfg["w_body_ang_vel"] == pytest.approx(-0.18)
        assert cfg["w_angular_momentum"] == pytest.approx(-0.018)

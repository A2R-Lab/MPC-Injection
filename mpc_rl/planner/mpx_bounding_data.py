    ...
    *,
    ...
    return {
        ...
        "controller": {
            ...
            "mpx_qtau_cost": (
                None if mpx_qtau_cost is None else float(mpx_qtau_cost)
            ),
            "mpx_qq_cost": (
...
def validate_acceptance_declaration(declaration: Mapping[str, Any]) -> dict[str, Any]:
    ...
    height_cost = controller["mpx_qp_height_cost"]
    if height_cost is not None and (
        not np.isfinite(float(height_cost)) or float(height_cost) < 0.0
    ):
        raise ValueError(
            "controller mpx_qp_height_cost must be null or non-negative"
        )
    vertical_velocity_cost = controller["mpx_qdp_vertical_cost"]
    if vertical_velocity_cost is not None and (
        not np.isfinite(float(vertical_velocity_cost))
        or float(vertical_velocity_cost) < 0.0
    ):
        raise ValueError(
    ...
    torque_cost = controller["mpx_qtau_cost"]
    if torque_cost is not None and (
        not np.isfinite(float(torque_cost)) or float(torque_cost) < 0.0
    ):
        raise ValueError("controller mpx_qtau_cost must be null or non-negative")
    joint_position_cost = controller["mpx_qq_cost"]
    if joint_position_cost is not None and (
        not np.isfinite(float(joint_position_cost))

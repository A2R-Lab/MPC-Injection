...
def _controller_config_with_overrides(
    ...
    if (
        ...
        and mpx_qdp_vertical_cost is None
        and mpx_qleg_vertical_cost is None
        and mpx_qtau_cost is None
        and mpx_qq_cost is None
        and mpx_qdq_cost is None
    ...
    vertical_velocity_cost = (
        float(np.asarray(config.Qdp)[2, 2])
        if mpx_qdp_vertical_cost is None
        else float(mpx_qdp_vertical_cost)
    )
    leg_vertical_cost = (
        float(np.asarray(config.Qleg)[2, 2])
        if mpx_qleg_vertical_cost is None
        else float(mpx_qleg_vertical_cost)
    )
    torque_cost = (
        float(np.asarray(config.Qtau)[0, 0])
        if mpx_qtau_cost is None
        else float(mpx_qtau_cost)
    )
    joint_position_cost = (
        float(np.asarray(config.Qq)[0, 0])
        if mpx_qq_cost is None
        else float(mpx_qq_cost)
    )
    joint_velocity_cost = (
        float(np.asarray(config.Qdq)[0, 0])
        if mpx_qdq_cost is None
        else float(mpx_qdq_cost)
    )
    ...
    if (
        not np.isfinite(vertical_velocity_cost)
        or vertical_velocity_cost < 0.0
    ):
        raise ValueError(
            "mpx_qdp_vertical_cost must be finite and non-negative"
        )
    if not np.isfinite(leg_vertical_cost) or leg_vertical_cost < 0.0:
        raise ValueError(
            "mpx_qleg_vertical_cost must be finite and non-negative"
        )
    if not np.isfinite(torque_cost) or torque_cost < 0.0:
        raise ValueError("mpx_qtau_cost must be finite and non-negative")
    if not np.isfinite(joint_position_cost) or joint_position_cost < 0.0:
        raise ValueError("mpx_qq_cost must be finite and non-negative")
    if not np.isfinite(joint_velocity_cost) or joint_velocity_cost < 0.0:
        raise ValueError("mpx_qdq_cost must be finite and non-negative")
    qp = config.Qp.astype(config.W.dtype).at[2, 2].set(height_cost)
    qdp = (
        config.Qdp.astype(config.W.dtype)
        .at[2, 2]
        .set(vertical_velocity_cost)
    )
    qleg = config.Qleg.astype(config.W.dtype)
    for index in range(2, 3 * int(config.n_contact), 3):
        qleg = qleg.at[index, index].set(leg_vertical_cost)
    qtau = config.Qtau.astype(config.W.dtype) * (
        torque_cost / float(np.asarray(config.Qtau)[0, 0])
    )
    qq = config.Qq.astype(config.W.dtype) * (
        joint_position_cost / float(np.asarray(config.Qq)[0, 0])
    )
    qdq = config.Qdq.astype(config.W.dtype) * (
        joint_velocity_cost / float(np.asarray(config.Qdq)[0, 0])
    )
...
def _controller_configuration_arrays(env, mpc) -> dict:
    ...
    return {
        ...
        "controller_qdp_vertical_cost": float(
            np.asarray(controller_config.Qdp, dtype=np.float64)[2, 2]
        ),
        "controller_qleg_vertical_cost": float(
            np.asarray(controller_config.Qleg, dtype=np.float64)[2, 2]
        ),
        "controller_qtau_cost": float(
            np.asarray(controller_config.Qtau, dtype=np.float64)[0, 0]
        ),
        "controller_qq_cost": float(
            np.asarray(controller_config.Qq, dtype=np.float64)[0, 0]
        ),
        "controller_qdq_cost": float(
            np.asarray(controller_config.Qdq, dtype=np.float64)[0, 0]
        ),
    ...
    try:
        ...
        generation_settings = {
            ...
            "controller": {
                ...
                "mpx_qdp_vertical_cost": controller_configuration[
                    "controller_qdp_vertical_cost"
                ],
                "mpx_qleg_vertical_cost": controller_configuration[
                    "controller_qleg_vertical_cost"
                ],
                "mpx_qtau_cost": controller_configuration[
                    "controller_qtau_cost"
                ],
                "mpx_qq_cost": controller_configuration[
                    "controller_qq_cost"
                ],
                "mpx_qdq_cost": controller_configuration[
                    "controller_qdq_cost"
                ],
...
def gen_traj_quadruped_dr(
    *,
    ...
    if acceptance_declaration is not None:
        ...
        mpx_qdp_vertical_cost = controller_declaration[
            "mpx_qdp_vertical_cost"
        ]
        mpx_qleg_vertical_cost = controller_declaration[
            "mpx_qleg_vertical_cost"
        ]
        mpx_qtau_cost = controller_declaration["mpx_qtau_cost"]
        mpx_qq_cost = controller_declaration["mpx_qq_cost"]
        mpx_qdq_cost = controller_declaration["mpx_qdq_cost"]
    ...
    while saved < num_trajectories:
        ...
        traj_data = generate_trajectory(
            ...
            mpx_qtau_cost=mpx_qtau_cost,
            mpx_qq_cost=mpx_qq_cost,
            mpx_qdq_cost=mpx_qdq_cost,
...
def main():
    ...
    parser.add_argument(
        "--mpx-qdp-vertical-cost",
        type=float,
        default=None,
        help="Optional instance-local MPX body vertical-velocity cost",
    )
    parser.add_argument(
        "--mpx-qleg-vertical-cost",
        type=float,
        default=None,
        help="Optional instance-local MPX per-foot vertical tracking cost",
    )
    parser.add_argument(
        "--mpx-qtau-cost",
        type=float,
        default=None,
        help="Optional instance-local MPX per-joint torque-effort cost",
    )
    parser.add_argument(
        "--mpx-qq-cost",
        type=float,
        default=None,
        help="Optional instance-local MPX per-joint position-reference cost",
    )
    parser.add_argument(
        "--mpx-qdq-cost",
        type=float,
        default=None,
        help="Optional instance-local MPX per-joint velocity cost",
    )
    parser.add_argument(
        "--mpx-robot-height-m",
        type=float,
    ...
    if args.acceptance_declaration is not None:
        ...
        acceptance_declaration = make_commissioning_acceptance_declaration(
            ...
            mpx_qtau_cost=args.mpx_qtau_cost,
            mpx_qq_cost=args.mpx_qq_cost,
            mpx_qdq_cost=args.mpx_qdq_cost,

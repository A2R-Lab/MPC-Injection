#!/usr/bin/env python3
"""Plot quadruped average motor torque magnitude trajectories."""
...
try:
    ...
    raise SystemExit(
        "matplotlib is required to plot torque trajectories. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot the per-timestep average motor torque magnitude from one or "
            "more saved quadruped rollouts."
        )
    )
    parser.add_argument(
        "--trajectory",
        ...
        help=(
            ...
            "Repeat this flag to overlay multiple trajectories."
        ),
    )
    parser.add_argument(
        "--output",
        ...
        help=(
            ...
            "plots/body_trajectory_plots/quadruped_torque_trajectories.png"
        ),
    )
    parser.add_argument(
        "--no-show",
...
def resolve_output_path(output_path: Path | None) -> Path:
    ...
    return DEFAULT_OUTPUT_DIR / "quadruped_torque_trajectories.png"
...
def compute_average_torque_magnitude(data: np.lib.npyio.NpzFile) -> tuple[np.ndarray, np.ndarray]:
    ...
    avg_torque_magnitude = avg_torque_magnitude[timestep_slice]

    if "sim_dt" in data.files:
        sim_dt = float(data["sim_dt"])
        x_axis = np.arange(timestep_slice.start, timestep_slice.stop, dtype=float) * sim_dt
    else:
        x_axis = np.arange(timestep_slice.start, timestep_slice.stop, dtype=float)

    return x_axis, avg_torque_magnitude


def make_trajectory_label(trajectory_path: Path) -> str:
    return f"{trajectory_path.parent.name}/{trajectory_path.stem}"
...
def main() -> None:
    ...
    for trajectory_path in trajectory_paths:
        ...
        x_axis, avg_torque_magnitude = compute_average_torque_magnitude(data)
        uses_time_axis = uses_time_axis and ("sim_dt" in data.files)
        ax.plot(
            x_axis,
            avg_torque_magnitude,
            linewidth=2.2,
            alpha=0.95,
    ...
    print(f"Saved torque trajectory plot to: {output_path}")
    plt.close(fig)
